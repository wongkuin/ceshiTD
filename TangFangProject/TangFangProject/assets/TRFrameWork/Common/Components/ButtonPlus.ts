...
export default class ButtonPlus extends cc.Button {
    ...
    @property({ tooltip: "音效路径", type: '', multiline: true })
    audioUrl = 'se1';
    ...
    _onTouchBegan(event) {
        ...
        if (this.openLongPress && !this.longPressFlag) {    // 开启长按
            ...
            this.node.emit('longclickTouchStart', this);
    ...
    _onTouchEnded(event) {
        if (!this.interactable || !this.enabledInHierarchy) return;
        this.node.emit('longclickTouchEnd');
        if (this["_pressed"] && this.longPressFlag) {
            ...
            SoundMgr.getInstance().playSound(this.audioUrl);
    ...
    /** 添加一个长按事件 */
    addLongClick(startFunc: Function, endFunc: Function, target: Object, longClickFunc: Function = null, longCancelFunc: Function = null) {
        ...
        this.node.on('longclickTouchStart', longClickFunc, target);
        this.node.on('longclickTouchEnd', longCancelFunc, target);
    }
}