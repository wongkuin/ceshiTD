...
export class Md5TS {
    ...
    private static onePassHasher = new Md5TS();

    private static _hex(x: any): string {
        const hc = Md5TS.hexChars;
        const ho = Md5TS.hexOut;
        let n;
        let offset;
        let j;
        let i;
    ...
    public start() {
        ...
        this._state.set(Md5TS.stateIdentity);
        return this;
    }
    ...
    public appendStr(str: string) {
        ...
        for (i = 0; i < str.length; i += 1) {
            ...
            if (bufLen >= 64) {
                this._dataLength += 64;
                Md5TS._md5cycle(this._state, buf32);
                bufLen -= 64;
                buf32[0] = buf32[16];
            }
        }
    ...
    public end(raw: boolean = false) {
        ...
        buf32.set(Md5TS.buffer32Identity.subarray(i), i);

        if (bufLen > 55) {
            Md5TS._md5cycle(this._state, buf32);
            buf32.set(Md5TS.buffer32Identity);
        }
        ...
        return raw ? this._state : Md5TS._hex(this._state);
    }
}
