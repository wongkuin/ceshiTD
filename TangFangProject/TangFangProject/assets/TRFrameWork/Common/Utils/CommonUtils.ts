...
export class CommonUtils {
    ...
    public static setItemSpriteFrame(sprite: cc.Sprite, url: string, successCB: (sprite: cc.Sprite) => void = null) {
        ...
        CocosHelper.loadResSync(url, cc.SpriteFrame).then((spriteFrame: any, err?: Error) => {
  ...
  * @returns 
  */
    public static addOutline(text: string, width: number, color: string): string {
    ...
    /**
     * @description: 将区间[a,b]分成n份
     * @param a 
     * @param b 
     * @param n 
     * @returns 
     */
    public static divideInterval(a: number, b: number, n: number): number[] {
        if (n <= 0) {
    ...
    //向量转弧度
    public static vectorToRadian(vector: cc.Vec2): number {
        return Math.atan2(vector.y, vector.x);
    }

    //弧度转向量
    public static radianToVector(radian: number): cc.Vec2 {
     ...
     * @param repeat 闪烁次数
     */
    public flashRedWithOpacity(node: cc.Node, repeat: number = 2) {
        cc.Tween.stopAllByTarget(node);
        node.color = cc.Color.WHITE.clone();
        node.opacity = 255;
        node.active = true;
        const original = {
        ...
        cc.tween(node)
            .parallel(  // 颜色和透明度同时变化
                cc.tween().to(0.1, { color: cc.Color.RED }),
                cc.tween().to(0.1, { opacity: 180 })
            )
            .parallel(
                cc.tween().to(0.1, { color: original.color }),
                cc.tween().to(0.1, { opacity: original.opacity })
            )
            .repeat(repeat)  // 重复2次（共闪烁3次）
            .call(() => {
                node.active = false;
            })
            .start();
    ...
    // 获取当前图片内存占用
    public static showTextureMemony() {
        // 计算每个像素的字节数
        let getBytesPerPixel = function (pixelFormat) {
            switch (pixelFormat) {
                ...
                case cc.Texture2D.PixelFormat.A8:
                    return 1;
                default:
                    console.warn('未知像素格式:', pixelFormat);
                    return 4; // 默认按RGBA8888处理
            }
        }

        // 计算单个纹理的内存占用
        let calculateTextureMemory = function (texture) {
            if (!texture || !texture.width || !texture.height) return 0;
            const bpp = getBytesPerPixel(texture.getPixelFormat());
            return texture.width * texture.height * bpp;
        }

        // 获取所有纹理内存总占用
        let getTotalTextureMemory = function () {
            const textures = new Set(); // 用于去重
            let totalMemory = 0;
            cc.assetManager.assets.forEach((element) => {
                if (element instanceof cc.Texture2D) {
                    // console.log('element instanceof cc.Texture2D', element);
                    if (element && !textures.has(element)) {
                        textures.add(element);
                        totalMemory += calculateTextureMemory(element);
                    }
                }
            });
        ...
        // 使用示例
        const totalMem = getTotalTextureMemory();
        console.log(`所有图片资源占用内存: ${totalMem} 字节 ≈ ${(totalMem / 1024 / 1024).toFixed(2)} MB`);
    }


    /**
     * @description: 隐藏spine骨骼
     * @param spine spine对象
     * @param boneName  骨骼名称
     */
    public static hideBoneByName(spine: sp.Skeleton, boneName: string) {
