...
import FromResMgr from "./FromResMgr";
...
export default class UIManager {
    ...
    public static getInstance(): UIManager {
        if (this.instance == null) {
            ...
            if (!scene) {
                console.error("Scene node not define.");
            }
    ...
    public async loadUIForm(form: IFormConfig): Promise<UIBase> {
        let uiBase = await this._loadForm(form.prefabUrl, form.bundleName);
        uiBase.fid = form.prefabUrl;
        if (!uiBase) {
    ...
    public async openForm(form: IFormConfig, params?: any, formData?: IFormData): Promise<UIBase> {
        ...
        let com = await this._loadForm(prefabPath, form.bundleName);
        if (!com) {
    ...
    public async closeForm(form: IFormConfig, params?: any, formData?: IFormData): Promise<boolean> {
        ...
        switch (com.formType) {
            case FormType.Screen:
                ...
                EventCenter.emit(EventType.SceneClosed, prefabPath);
                break;
        ...
        if (com.formData) {
            com.formData.onClose && com.formData.onClose(params);
        }
     ...
     */
    private async _loadForm(prefabPath: string, bundleName: string): Promise<UIBase> {
        let com = this._allForms[prefabPath];
        if (com) return com;
        return new Promise((resolve, reject) => {
            ...
            this._doLoadUIForm(prefabPath, bundleName).then((com: UIBase) => {
     ...
     * @param prefabPath 
     */
    private async _doLoadUIForm(prefabPath: string, bundleName: string): Promise<UIBase> {
        let prefab = await FromResMgr.getInstance().loadFormPrefab(prefabPath, bundleName);
    ...
    private async enterToScreen(fid: string, params: any) {
        ...
        for (let key in this._showingForms) {
            //console.warn(`关闭窗口 ${key}`);
            if (key != "prefabs/SceneLogin")
        ...
        AdapterMgr.getInstance().adapteByType(AdapterType.StretchHeight | AdapterType.StretchWidth, com.node);
    ...
    private destoryForm(com: UIBase) {
        ...
        com.node.destroy();
        // 销毁prefab以及依赖的资源
        FromResMgr.getInstance().destoryFormPrefab(com.fid);
        // 销毁动态加载的资源
        FromResMgr.getInstance().destoryDynamicRes(com.fid);
        // 从allmap中删除
    ...
    /** 获得Component */
    public getForm(fId: string): UIBase {
