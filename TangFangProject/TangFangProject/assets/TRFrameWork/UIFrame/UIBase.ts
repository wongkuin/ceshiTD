...
export default class UIBase extends cc.Component {
    ...
    /** 设置是否挡住触摸事件 */
    protected _blocker: cc.BlockInputEvents = null;
    ...
    public async loadRes<T extends cc.Asset>(url: string, type?: { prototype: T }, bundleName?: string): Promise<T> {
        return await FromResMgr.getInstance().loadDynamicRes(url, type, bundleName, this.fid);
    }

    public async releaseRes(res: cc.Asset) {
        return await FromResMgr.getInstance().releaseDynamicRes(res, this.fid);
    }

    public async loadSpirteFrame(url: string, sp: cc.Sprite, bundleName?: string) {
        return this.loadRes(url, cc.SpriteFrame, bundleName).then((res: cc.SpriteFrame) => {
