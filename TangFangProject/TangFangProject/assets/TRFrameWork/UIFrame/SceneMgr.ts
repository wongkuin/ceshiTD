...
class SceneMgr {
    ...
    public getCurrScene(): UIScreen {
        return UIManager.getInstance().getForm(this._currScene.prefabUrl) as UIScreen;
    }
    ...
    private async openLoading(formConfig: IFormConfig, params: any, formData: IFormData) {
        ...
        if (!form) return;
        await TipsMgr.open(formConfig, params, formData);
    }
    private async closeLoading(formConfig: IFormConfig) {
        let form = formConfig || SysDefine.defaultLoadingForm;
        if (!form) return;
        await TipsMgr.close(formConfig);
    }
