...
class ToastMgr {
    ...
    public async close(com: UIToast, params: any) {
        ...
        if (!this._pools[com.prefabUrl]) return;
        this._pools[com.prefabUrl].free(com);

        let arr = this._showingToast[com.prefabUrl];
        //-----------这部分代码有问题
        // if (!arr) return;
    ...
    private async genPool(form: IFormConfig) {
        // 对pool创建一个独立结点
        // todo...
        let prefab = await FromResMgr.getInstance().loadFormPrefab(form.prefabUrl, form.bundleName);
        let pool = this._pools[form.prefabUrl] = new Pool(() => {
    ...
    public clearToasts(prefabUrl: string) {
        ...
        // 销毁prefab以及依赖的资源
        FromResMgr.getInstance().destoryFormPrefab(prefabUrl);
