 ...
 */
export default class FromResMgr extends Singleton<FromResMgr> {
    // private static instance: FromResMgr = null;
    // public static get inst() {
    //     if (this.instance === null) {
    //         this.instance = new FromResMgr();
    //     }
    ...
    /** 加载窗体 */
    public async loadFormPrefab(fid: string, bundleName: string) {
        if (this._prefabs[fid]) return this._prefabs[fid];
        let { res, deps } = await this._loadResWithReference(fid, cc.Prefab, bundleName);
    ...
    public async loadDynamicRes<T extends cc.Asset>(url: string, type: { prototype: T }, bundleName: string, tag: string): Promise<T> {
        let { res, deps } = await this._loadResWithReference<T>(url, type, bundleName);
        if (!this._dynamicTags[tag]) {
            this._dynamicTags[tag] = [];
        }
        // console.log(`loadDynamicRes tag:${tag} deps:${deps}`);
        this._dynamicTags[tag].push(...deps);
        return res;
    }

    /** 释放动态资源 */
    public releaseDynamicRes(res: cc.Asset, tag: string) {
        if (!this._dynamicTags[tag]) {       // 销毁
            return false;
        }

        let idx = this._dynamicTags[tag].findIndex((s) => {
            return s === res['_uuid'];
        });
        ...
        let data = this._dynamicTags[tag].splice(idx, 1);
        this._destoryAssets(data);
    }
    ...
    /** 加载资源并添加引用计数 */
    private async _loadResWithReference<T extends cc.Asset>(url: string, type: { prototype: T }, bundleName?: string): Promise<{ res: T, deps: string[] }> {
        let data = await CocosHelper.loadResSync(url, type, bundleName, this._addTmpAssetsDepends.bind(this));
        if (data.err) {
        ...
        this._clearTmpAssetsDepends();
        let deps = cc.assetManager.dependUtil.getDepsRecursively(data.asset['_uuid']) || [];
        deps.push(data.asset['_uuid']);
