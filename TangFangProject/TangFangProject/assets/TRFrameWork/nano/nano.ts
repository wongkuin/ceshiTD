...
export type NanoInitParams = {
  host: string,
  port: number,
  path: string,
  ws: string, //wss or ws
  reconnect: boolean,
  encode?: (reqId, route, msg) => Uint8Array<ArrayBuffer>,
  decode?: (data) => any,
  user?: any,
  encrypt?: boolean,
  handshakeCallback?: (data) => void
...
export default class nano extends cc.EventTarget {
  ...
  init(params: NanoInitParams, cb) {
    ...
    var url = params.ws + '://' + host;// wss://' + host;
    if (port) {
      url += ':' + port;
    }
  ...
  connect(param: NanoInitParams, url, cb) {
