...
import { BTNode, BTNodeData, BTNodeDef } from "./node";
import { Index } from "./nodes/actions/BTindex";
...
import { BTTreeData } from "./tree";


// eslint-disable-next-line @typescript-eslint/no-explicit-any
export type Constructor<T, A extends any[] = any[]> = new (...args: A) => T;
export type NodeContructor<T extends BTNode> = Constructor<T, ConstructorParameters<typeof BTNode>> & {
    descriptor: BTNodeDef;
};
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export abstract class BTContext {
    readonly nodeDefs: Record<string, DeepReadonly<BTNodeDef>> = {};
    readonly nodeCtors: Record<string, NodeContructor<BTNode>> = {};
    readonly trees: Record<string, BTNode> = {};
    ...
    abstract loadTree(path: string): Promise<BTNode>;
    ...
    registerNode<T extends BTNode>(cls: NodeContructor<T>) {
        // console.log(cls)
        const descriptor = cls.prototype.descriptor;
        this.nodeDefs[descriptor.name] = descriptor;
        this.nodeCtors[descriptor.name] = cls;
    }

    protected _createTree(treeCfg: BTTreeData) {
        const traverse = (cfg: BTNodeData) => {
            cfg.tree = treeCfg;
            cfg.input ||= [];
            cfg.output ||= [];
            cfg.children ||= [];
        ...
        return BTNode.create(this, treeCfg.root);
    }
}