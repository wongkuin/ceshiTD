...
func main() {
	...
	excels := deserialezeExcelJson(bytes)
	...
	if err := ioutil.WriteFile(*outFile, []byte(fileContent), 0777); err != nil {
		fmt.Println(err)
		return
	}
	...
	bytes, err = ioutil.ReadFile(fmPath)
	if err != nil {
		fmt.Println(err)
		return
	}

	fm := deserialezeFMJson(bytes)
	baseURL := "https://oss.fiveteam.top/请填入路径/"
	// for _, value := range fm.ConfigsNeedPreload {
	// 	if value.Name == keys[0] {
	// 		baseURL = strings.Replace(value.Url, "configfm.json", "", 1)
	// 	}
	// }
	...
	json.Indent(&str, bytes, "", "\t")

	if err := ioutil.WriteFile(fmPath, str.Bytes(), 0777); err != nil {
		fmt.Println(err)
		return
	}
...
export default class ConfigMgr extends Singleton<ConfigMgr> {
    ...
    public loadAll(option?: { remoteUrl: string, remoteKeys?: Array<string> }): Promise<void> {
        ...
        return new Promise(resolve => {
            ...
            this.keyMap.forEach((key, value) => {
             ...
             CocosHelper.loadRes(loadPath + key, cc.JsonAsset,null, (err, asset: cc.JsonAsset) => {
                    if (err) console.log(err);
                    else this.dataMap.set(key, asset.json);
                    // console.log("load config " + key + " success");
                    count--;
    ...
    */
	...
	if err := ioutil.WriteFile(*mgrFile, []byte(fileContent), 0777); err != nil {
		fmt.Println(err)
		return
	}
...
func deserialezeExcelJson(bytes []byte) []Excel {
	excels := make([]Excel, 0)
...
func deserialezeFMJson(bytes []byte) *FM {
	var fm FM
